...
//!
//! `.url`, `.webloc`, `.desktop` (`Type=Link`) and macOS `BookmarkData` are
//! four spellings of one idea, and `plan/PLAN.md` §4.10 wants them behind a
//! single type. This crate is that type's home for now: it is the smallest and
//! oldest member of the family, so nothing has to depend on anything bigger to
//! get at it.
//!
//! `rclip-desktop-entry` and `rclip-uri-list` carry a byte-identical copy
//! rather than a dependency, because codec crates in this workspace do not
//! depend on each other.
//
// TODO(phase-4): hoist this into `rclip-core` and have all four shortcut crates
// re-export it, replacing the mirrored copies.
