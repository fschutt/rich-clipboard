...
//!
//! `.url`, `.webloc`, `.desktop` (`Type=Link`), `.lnk` and `text/uri-list` are
//! five encodings of one idea: a small file that names a destination. They
//! disagree about syntax and agree about meaning, so the meaning lives here
//! and each codec crate re-exports it. Before this was hoisted, four crates
//! carried byte-identical copies and a consumer holding two of them had two
//! incompatible types for the same concept.
//!
//! Classification is deliberately syntactic. Deciding whether a path exists,
